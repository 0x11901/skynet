...
pack(lua_State *L, int opcode) {
    ...
    frame_header[pos++] = FRAME_SET_FIN(1) | FRAME_SET_OPCODE(opcode);
    if (len < 126)
    {
        frame_header[pos++] = FRAME_SET_MASK(0) | FRAME_SET_LENGTH(len, 0);
    }
...
static int 
lpackText(lua_State *L) {
	return pack(L, 0X01);
}

static int 
lpackPing(lua_State *L) {
	return pack(L, 0X09);
}

static int 
lpackPong(lua_State *L) {
	return pack(L, 0X0A);
}

static int 
lpackBinary(lua_State *L) {
	return pack(L, 0X02);
}
...
LUAMOD_API int
luaopen_skynet_websocketnetpack(lua_State *L) {
	luaL_checkversion(L);
    luaL_Reg l[] = {
        { "pop", lpop },
        { "pack", lpack },
        { "packText", lpackText },
        { "packBinary", lpackBinary },
        { "packPing", lpackPing },
        { "packPong", lpackPong },
        { "clear", lclear },
        { "tostring", ltostring },
